...
bot state, and notification delivery. Administrator identities, sessions and
...
| --- | --- | --- |
| Admin API | `admin/` | Session-authenticated, RBAC-protected control plane for users, finance, referrals, configs, fleet and observability |
...
| Admin frontend | `admin_frontend/` | React/TypeScript operations console |
...
Manager 1.3 inventory and aggregate fleet status can be audited read-only and conditionally cached with an
...
| `ADMIN_USERNAME` | Username used to bootstrap the first persisted owner | `admin` |
| `ADMIN_PASSWORD_HASH` | bcrypt hash used only for first-owner bootstrap | Required for initial login |
| `ADMIN_COOKIE_SECURE` | Restrict admin session and CSRF cookies to HTTPS | `true` |
| `ADMIN_LEGACY_API_ENABLED` | Temporary rollback switch for the old Bearer API | `false` |
| `ADMIN_TRUSTED_PROXY_CIDRS` | Proxies whose forwarded client chain may be trusted | Loopback/Compose CIDRs |
| `ADMIN_FLEET_POLL_INTERVAL_SECONDS` | Background Manager status interval | `60` |
| `ADMIN_FLEET_STATUS_STALE_SECONDS` | Age after which a fleet snapshot is stale | `300` |
| `PROMETHEUS_API_URL` | Internal Prometheus API used by the monitoring page | `http://prometheus:9090` |
...
Nginx endpoint using relative `/api/admin/v1` routes. No browser-visible API
credential or `admin_frontend/.env` file is required by Compose.

To test only the admin backend without starting Telegram or financial workers:

```bash
docker compose up --build db redis migrations admin
```
...
SQLite covers fast unit and service tests. Locking and timestamp-contract tests
use a real, disposable PostgreSQL database when `POSTGRES_TEST_URL` is supplied:
...
POSTGRES_TEST_URL=postgresql+asyncpg://vpn:vpn@localhost:5432/vpn_test \
  pytest tests/test_billing_concurrency.py tests/test_vpn_lifecycle_concurrency.py \
    tests/test_telegram_update_concurrency.py tests/test_admin_fleet_concurrency.py \
    tests/test_admin_analytics_postgres.py
```
...
The versioned API is exposed under `/api/admin/v1` through Nginx. Login creates
an opaque, hashed PostgreSQL session in an HttpOnly `SameSite=Strict` cookie;
mutations also require the matching `X-CSRF-Token`. Roles map to explicit
permissions, and balance/fleet mutations require optimistic guards,
idempotency keys, a reason and an immutable audit event. Manager API keys are
encrypted at rest and never returned. The old Bearer endpoints are disabled by
default and in production. See [`admin/README.md`](admin/README.md) for the
endpoint and role contract.
...
After a successful bot canary for the exact main SHA, the separate manual
`Activate admin hub` workflow starts only admin, SPA, loopback proxy and
