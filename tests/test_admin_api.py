...
async def test_user_endpoints(monkeypatch, sessionmaker):
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        resp = await client.post(
            "/api/users", json={"tg_id": 1, "username": "alice"}, headers=headers
        )
        assert resp.status_code == 200
        alice = resp.json()

        resp = await client.post(
            "/api/users", json={"tg_id": 2, "username": "bob"}, headers=headers
        )
        assert resp.status_code == 200

        # list with filter
        resp = await client.get(
            "/api/users", params={"username": "alice"}, headers=headers
        )
        assert resp.status_code == 200
        data = resp.json()
        ...
        # Retrying the same HTTP mutation with a stable key must not move money
        # twice.  The header is optional to keep the existing frontend contract.
        mutation_headers = {**headers, "Idempotency-Key": "admin-request-1"}
        first = await client.post(
            f"/api/users/{alice['id']}/topup",
            json={"amount": 10},
        ...
        assert first.json()["balance"] == duplicate.json()["balance"] == 10
...
async def test_config_list(monkeypatch, sessionmaker):
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        resp = await client.get(
            "/api/configs", params={"owner_id": user.id}, headers=headers
        )
        assert resp.status_code == 200
        data = resp.json()
